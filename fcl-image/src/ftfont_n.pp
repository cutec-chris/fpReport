...
uses SysUtils, Classes, FPCanvas, fpimgcmn, freetype_n, freetypeh_n;
...
begin
  if assigned (FLastText) then
    begin
    if FLastText.InheritsFrom(TStringBitMaps) or (CompareStr(TStringBitMaps(FLastText).Text,aText) <> 0) then
      begin
      FLastText.Free;
      b := true;
      end
