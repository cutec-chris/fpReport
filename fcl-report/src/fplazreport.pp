...
Type
  ...
  Public
    ...
    function FixDataFields(aFieldName : string) : string;virtual;
...
begin
    ...
    if (copy(NodeName,0,6)='Object') and (NodeName<>'ObjectCount') then
      ...
      'TfrBandView':
        begin
          ...
          if aObj = FMasterData then
            aPage.Data := TFPReportCustomBandWithData(aObj).Data;
        end;
      'TfrMemoView':
  ...
  for j := 0 to Result.BandCount-1 do
    if (Result.Bands.Items[j] is TFPReportCustomBandWithData)
    and (TFPReportCustomBandWithData(Result.Bands.Items[j]).Data=nil) then
      begin
        TFPReportCustomBandWithData(Result.Bands.Items[j]).Data := aPage.Data;
      end;
end;
